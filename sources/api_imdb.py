...
def progress_bar(total, current, description, prefix):
    ...
    print("\r{0}: |{1}| {2}% [{3}] ".format(prefix, bar, percent, description))
...
class API_IMDb():
    ...
    def download_reviews(self, save):
        ...
        for season, episodes in zip(serie.keys(), serie.values()):
            ...
            for episode_number, episodeObject in zip(episodes.keys(), episodes.values()):
                ...
                current_episode += 1
                progress_bar(show['number of episodes'], current_episode, "Now downloading Season {0} Episode {1}".format(season, episode_number), "Complete")
    ...
    def download_number_of_votes(self):
        """
